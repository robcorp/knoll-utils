...
#_(def unique-color-names (into (sorted-set)
                              (spctr/select [spctr/ALL :FabricColors spctr/ALL :ColorName] @fabrics)))
...
(s/def ::FabricUses (s/coll-of ::FabricUse :max-count 5))
...
(s/def ::CuttingDirection  (s/nilable #{""
                                        ...
                                        "RR - Railroaded"}))
(s/def ::AverageBoltsPerYard  #{"N/A" "15" "21" "22" "23" "25" "250" "27" "30" "31" "32" "33" "35" "36" "37"
...
(s/def ::WeightFormatted (s/nilable (into #{"N/A" "2.6 oz."} (map #(str % " oz." ) (range 1.0 44.0 0.25)))))
(s/def ::UseName #{"" "Cubicle" "Drapery" "HC" "Panel" "Upholstery" "Wallcovering"})
(s/def ::UseCode #{"" "C" "D" "HC" "K" "QK" "W" "WC"})
...
(s/def ::TestName #{"AATCC  TM 30 anti-microbial & mildew resistant" "AATCC TM 107 colorfastness to water" "AATCC TM 147 anti-bacterial"
                    ...
                    "NFPA 701-2004 TM#1 with Topical Treatment" "NFPA 701-2004 TM#1, as stocked" "REACH" "Seam Slippage Warp" "Seam Slippage Warp 2" "Seam Slippage Warp 4"
                    "Seam Slippage Weft" "Seam Slippage Weft 2" "Seam Slippage Weft 4" "Tensile Strength Warp" "Tensile Strength Warp 2" "Tensile Strength Warp 3"
                    "Tensile Strength Weft" "Tensile Strength Weft 2" "Tensile Strength Weft 3" "Tongue Tear ASTM D2261" "Tongue Tear D2261" "Wyzenbeek Published"
                    "Wyzenbeek Warp"})
...
(s/def ::NetPriceFormatted (into #{} (map #(str "$" % " USD") (range 0 360))))
(s/def ::CanadianPriceFormatted (into #{} (map #(str "$" % " CAD") (range 0 470))))
...
(s/def ::Weight (into #{"N/A" "2.6"} (map str (range 0.0 44.0 0.25))))
...
#_(def textile-service-url "http://knlecomprd2.knoll.com:9082/textiles/")
(def textile-service-url "http://knlecomprd1.knoll.com/textiles/")

(defonce fabrics (future (decode
                      (:body @(http/get (str textile-service-url "fabrics"))) true)))
...
(defn sort-contents-by-percentage [fabrics]
  (spctr/transform [spctr/ALL :Contents] #(sort-by :Percentage %) fabrics))

(defn get-textile-attribute-sorted-set [path]
  (into (sorted-set) (spctr/select path @fabrics)))
